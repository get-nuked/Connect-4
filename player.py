...
class Player:
	...
	def hasImmediateWin(self, gameBoard: Board, player: str) -> bool: 
		"""
		Docstring for hasImmediateWin
		
		:param self: Description
		:param gameBoard: current game board state
		:param player: the player wished to check
		"""
		for col in range(gameBoard.numColumns): # adds a disk in each column and checks fo a win
	...
	def getMove(self, gameBoard: Board) -> int:  # minimax
		"""
		Docstring for getMove
		
		:param self: Description
		:param gameBoard: current game board state
		:type gameBoard: Board
		:return: the most optimal move decided by minimax without pruning 
		:rtype: int
		"""
		bestValue = -math.inf
		bestMove = -1
		...
		for col in order:
			if gameBoard.colFills[col] < gameBoard.numRows:
				self.numExpanded += 1
				gameBoard.addPiece(col, "X")
				moveValue = self.minimax(1, gameBoard, False) # calls minimax recursively for each branch
				gameBoard.removePiece(col)
	...
	def minimax(self, depth: int, gameBoard: Board, isMaximizing: bool) -> int:
		"""
	...
	def getMoveAlphaBeta(self, gameBoard: Board) -> int:  # minimax with alpha-beta pruning
		"""
	...
	def numberOfFloatingSpaces(self, gameBoard: Board, coordinates: list[tuple[int, int]]) -> int:
		"""
	...
	def evaluationFunction(self, gameBoard: Board) -> int:
		"""
