...
class PlayerOpponent:
	...
	def getMove(self, gameBoard):  # minimax
		...
		# if not found, uses heuristic 
		for col in order:
			if gameBoard.colFills[col] < gameBoard.numRows:
				self.numExpanded += 1
				gameBoard.addPiece(col, "O")
				moveValue = self.minimax(1, gameBoard, True)
				gameBoard.removePiece(col)
	...
	def evaluationFunction(self, gameBoard):
				...
				if self.hasAtleastOnePlayableSpace(gameBoard, [(row, col + i) for i in range(gameBoard.winNum)]):
					...
					if (x_count > 0) ^ (o_count > 0):
						if x_count == gameBoard.winNum:  # x wins
							...
							weight = 1.5 * (10 ** x_count) * 0.5 ** self.numberOfFloatingSpaces(gameBoard, [(row, col + i) for i in range(gameBoard.winNum)])  # adds square of number of X pieces to value
						else:
							weight = -1 * (10 ** o_count) * 0.5 ** self.numberOfFloatingSpaces(gameBoard, [(row, col + i) for i in range(gameBoard.winNum)])  # subtracts square of number of O pieces from value
				...
				if self.hasAtleastOnePlayableSpace(gameBoard, [(row + i, col) for i in range(gameBoard.winNum)]):
					...
					if (x_count > 0) ^ (o_count > 0):
						if x_count == gameBoard.winNum:
							...
							weight = 1.5 * (10 ** x_count) * 0.5 ** self.numberOfFloatingSpaces(gameBoard, [(row + i, col) for i in range(gameBoard.winNum)])
						else:
							weight = -1 * (10 ** o_count) * 0.5 ** self.numberOfFloatingSpaces(gameBoard, [(row + i, col) for i in range(gameBoard.winNum)])
				...
				if self.hasAtleastOnePlayableSpace(gameBoard, [(row + i, col + i) for i in range(gameBoard.winNum)]):
					...
					if (x_count > 0) ^ (o_count > 0):
						if x_count == gameBoard.winNum:
							...
							weight = 1.5 * (10 ** x_count) * 0.5 ** self.numberOfFloatingSpaces(gameBoard, [(row + i, col + i) for i in range(gameBoard.winNum)])
						else:
							weight = -1 * (10 ** o_count) * 0.5 ** self.numberOfFloatingSpaces(gameBoard, [(row + i, col + i) for i in range(gameBoard.winNum)])
		...
		# additional heuristic: center column controlcolFill
		center = gameBoard.numColumns // 2
		for col in range(gameBoard.numColumns):
